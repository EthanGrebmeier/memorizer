...
function clickHandle(x,y){
    ...
    if (clickable){
        if (gameArray[x][y].clicked){
            ...
            gameArray[x][y].element.style.background = "#43eb8a";
            clicked -= 1;
        } else if (clicked < selected){
            gameArray[x][y].clicked = true;
            gameArray[x][y].element.style.background = "#83a2ff";
            clicked += 1; 
        }
...
function initialize(){
    ...
    for (let row = 0; row <= rows; row++){
        ...
        for (let col = 0; col <= cols; col++){
            let square = document.createElement(`span${row}${col}`);
...
function toIntermission(){
    ...
    for (let r = 0; r <= rows; r++){
        for (let c = 0; c <=cols; c++){
            gameArray[r][c].selected = false;
            gameArray[r][c].clicked = false;
        }
    }
    console.log(gameArray);
    play = true;
